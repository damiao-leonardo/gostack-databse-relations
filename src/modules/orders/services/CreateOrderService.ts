...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    const checkCustomerId = await this.customersRepository.findById(
      customer_id,
    );

    if (!checkCustomerId) {
      throw new AppError('Error, Customer does not exist');
    }
    ...
    if (!existentProducts.length) {
      throw new AppError('Products do not exists');
    }
    ...
    if (checkInexistentProducts.length) {
      throw new AppError(
        `Could not find product ${checkInexistentProducts.map(product => {
          if (product.id === checkInexistentProducts[0].id) {
            return `[${product.id}`;
          }
